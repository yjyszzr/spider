...
for ul in shops:
    try:
        ...
        tel = re.search('href="tel:(.*?)"',res).group(1)
        shop_name = re.search('<h1.*?class="shop-name">(.*?)</h1>',res).group(1)
        sql = "insert into dazhong(city,tel,shop_name,class) values('{}','{}','{}','{}')".format('北京',tel,shop_name,cla)
        # cur.execute(sql)
        # db.commit()
